...
module.exports = function(gemini, options) {
    ...
    gemini.on('startRunner', function(runner) {
        ...
        runner.on('testResult', function(data) {
            ...
            var link = `${gemini.config.forBrowser(data.suite.browsers[0]).rootUrl}/${data.suite.url.replace(/^\//, '')}`
                        .replace('?screenshotTest', '');
            
            tsm.testMetadata({
                testName: testName,
                name: 'View in documentation',
                type: 'link',
                value: link
            });
